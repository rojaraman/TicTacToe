...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        if(gridAt(1 ,1) != '-') {
            if(gridAt(0,0) == gridAt(1,1)){
                if(gridAt(2,2) == gridAt(1,1)){
                    if(gridAt(1,1) == 'x'){
                        result = "X wins";
                        return result;
                    }
            ...
            }else if(gridAt(1,1) == gridAt(0,2)){
                if(gridAt(1,1) == gridAt(2,0)){
                    if(gridAt(1,1) == 'x'){
                        result = "X wins";
                        return result;
                    }
        ...
        for(int i=0;i<=2;i++){ // 0
            for(int j=0;j<2;j++){ //0 1 2
                if(gridAt(i ,j) != '-'){ // say [0,0] is not empty
                    if(gridAt(i,j) == gridAt(i,j+1)){
                        if(gridAt(i,j+2) == gridAt(i,j)){
                            if(gridAt(i,j) == 'x'){
                                result = "X wins";
                                return result;
                            }
        ...
        for(int j=0;j<=2;j++){ // 0
            for(int i=0;i<2;i++){ //0 1 2
                if(gridAt(i ,j) != '-'){ // say [0,0] is not empty
                    if(gridAt(i,j) == gridAt(i+1,j)){
                        if(gridAt(i+2,j) == gridAt(i,j)){
                            if(gridAt(i,j) == 'x'){
                                result = "X wins";
                                return result;
                            }
